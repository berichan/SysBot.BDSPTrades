...
namespace SysBot.Tests
{
    public class QueueTests
    {
        ...
        private static void EnqueueTest<T>() where T : PKM, new()
        {
            ...
            var queue = info.Hub.Queues.GetQueue();
            ...
            var status = info.CheckPosition(t1.UserID);
            ...
            status = info.CheckPosition(t1.UserID);
        ...
        private class MockExecutor<T> : PokeRoutineExecutor<T> where T : PKM, new()
        {
            ...
            public override Task<T> ReadPokemonPointer(long[] jumps, int size, CancellationToken token) => Task.Run(() => new T(), token);
        }
        ...
        private static TradeEntry<T> GetTestTrade<T>(int tag, bool favor) where T : PKM, new()
        {
            ...
            return new TradeEntry<T>(d3, (ulong)tag, PokeRoutineType.BDSPLinkTrade, $"Test Trade {tag}");
        }
